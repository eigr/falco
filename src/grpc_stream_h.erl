...
init(StreamID, Req = #{ref := Ref}, Opts) ->
    ...
    {[{spawn, Pid, Shutdown} | Commands], #state{next = Next, ref = Ref, pid = Pid, expect = Expect}}.
...
% Falco: We don't pass auto, but treat it as auto
data(
    StreamID,
    IsFin,
    Data,
...
info(
    StreamID,
    Info = {'EXIT', Pid, {{request_error, Reason, _HumanReadable}, _}},
    State = #state{pid = Pid}
) ->
    Status =
        case Reason of
...
info(
    StreamID, Info = {read_body, Pid, Ref, auto, infinity}, State = #state{read_body_buffer = <<>>}
) ->
    do_info(StreamID, Info, [], State#state{
        read_body_pid = Pid,
        read_body_ref = Ref,
...
info(
    ...
    IsFin =:= fin; byte_size(Buffer) >= Length
->
    send_request_body(Pid, Ref, IsFin, BodyLen, Buffer),
...
info(
    StreamID,
    Info = {read_body_timeout, Ref},
    State = #state{
        read_body_pid = Pid,
        read_body_ref = Ref,
        read_body_is_fin = IsFin,
        read_body_buffer = Buffer,
        body_length = BodyLen
    }
) ->
    send_request_body(Pid, Ref, IsFin, BodyLen, Buffer),
...
info(StreamID, Inform = {inform, Status, _}, State0) ->
    State =
        case cow_http:status_to_integer(Status) of
...
info(StreamID, Data0 = {data, Pid, _, _}, State0 = #state{stream_body_status = Status}) ->
    ...
    do_info(StreamID, Data, [Data], State);
info(StreamID, Alarm = {alarm, Name, on}, State) when
    Name =:= connection_buffer_full; Name =:= stream_buffer_full
->
    do_info(StreamID, Alarm, [], State#state{stream_body_status = blocking});
info(
    StreamID,
    Alarm = {alarm, Name, off},
    State = #state{stream_body_pid = Pid, stream_body_status = Status}
) when
    Name =:= connection_buffer_full; Name =:= stream_buffer_full
->
    _ =
    ...
    do_info(StreamID, Alarm, [], State#state{
        stream_body_pid = undefined, stream_body_status = normal
    });
