defmodule Falco.ChannelTest do
  use ExUnit.Case
  alias Falco.Test.ClientAdapter
  alias Falco.Channel

  test "connect/2 works for insecure" do
    {:ok, channel} = Falco.Stub.connect("10.1.0.0:50051", adapter: ClientAdapter)
    assert %Channel{host: "10.1.0.0", port: 50051, scheme: "http", cred: nil} = channel
  end

  test "connect/2 works for ssl" do
    cred = %{ssl: []}
    {:ok, channel} = Falco.Stub.connect("10.1.0.0:50051", adapter: ClientAdapter, cred: cred)
  ...
  test "connect/2 allows setting default headers" do
    ...
    {:ok, channel} =
      Falco.Stub.connect("10.1.0.0:50051", adapter: ClientAdapter, headers: headers)
